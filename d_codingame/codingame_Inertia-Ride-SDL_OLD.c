...
int getPosFin( t_ride * tRide, t_sdl * tSdl, SDL_Point * pts_segs, int inertia, int count, int printRide )
{
	t_seg * tSeg = tRide -> first;
	...
	while(tSeg!=NULL)
	{
		...
		if( printRide == TRUE ){ printRow(tSdl,pts_segs,ball,count,inertia+1); }
...
int main(int argv,char ** args)
{
	...
	int posFin = getPosFin(tRide,tSdl,pts_segs,inertia,W+1,printSDL);
	printf("%d\n",posFin);
